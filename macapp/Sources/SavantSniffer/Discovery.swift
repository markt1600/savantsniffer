...
final class DiscoveryModel: ObservableObject {
    ...
    func detectSubnet() {
        // Derive /24 from the primary IPv4 without sending anything.
    ...
    func buildCommand() {
        ...
        switch tool {
        case "arp-scan":
            scanCommand = "sudo arp-scan \(subnet)"
            needsSudo = true
            note = "Fast layer-2 scan; returns IP + MAC + vendor."
        case "nmap":
            scanCommand = "sudo nmap -sn \(subnet)"
            needsSudo = true
            note = "Ping sweep; run as root to populate MAC addresses."
        default:
            scanCommand = "arp -a"
            needsSudo = false
    ...
    func runScan() {
        ...
        Task.detached {
            ...
            switch tool {
            case "arp-scan":
                if let p = Shell.which("arp-scan") {
                    result = Shell.run(p, [subnet])
                } else { result = Shell.Result(stdout: "", stderr: "arp-scan not found", code: -1) }
            case "nmap":
                if let p = Shell.which("nmap") {
                    result = Shell.run(p, ["-sn", subnet])
                } else { result = Shell.Result(stdout: "", stderr: "nmap not found", code: -1) }
            default:
                if let p = Shell.which("arp") {
                    result = Shell.run(p, ["-a"])
                } else { result = Shell.Result(stdout: "", stderr: "arp not found", code: -1) }
            }
            let parsed = Self.parse(tool: tool, text: result.stdout + "\n" + result.stderr)
            await MainActor.run {
                self.hosts = parsed.sorted { rank($0.classification) < rank($1.classification) }
                self.scanning = false
                if parsed.isEmpty && !result.stderr.isEmpty { self.lastError = result.stderr }
            }
        }
    }

    static func parse(tool: String, text: String) -> [DiscoveredHost] {
        ...
        if tool == "arp-scan" {
            ...
            for line in lines {
                if let r = line.range(of: #"(\d+\.\d+\.\d+\.\d+)"#, options: .regularExpression),
                   line.contains("Nmap scan report") {
                    currentIP = String(line[r])
                }
                if line.contains("MAC Address:"),
                   let macR = line.range(of: #"([0-9A-Fa-f:]{17})"#, options: .regularExpression),
                   let ip = currentIP {
            ...
            for line in lines {
                ...
                out.append(host(String(line[ipR]), String(line[macR]).replacingOccurrences(of: "-", with: ":"), ""))
            }
        }
        return out
    }

    private static func host(_ ip: String, _ mac: String, _ vendor: String) -> DiscoveredHost {
    ...
    private static func isMAC(_ s: String) -> Bool {
